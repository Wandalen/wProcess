...
function startOptionOutputPiping( test )
{
  ...
  /* qqq2 for Yevhen : poor tests! see no console.error() cases! | aaa : Fixed. */
  /* qqq2 for Yevhen : poor tests! see no multiline output! | aaa : Fixed. */
}
...
function startSingleOptionDry( test )
{
  ...
  modes.forEach( ( mode ) => a.ready.then( () => run({ mode, sync : 1, deasync : 0 }) ) ); /* qqq2 for Yevhen : was broken! */
...
function exitReason( test )
{
  ...
  function run( mode )
  {
    ...
    ready.then( () =>
    {
      ...
      let testAppPath = a.program({ routine : testAppSignal, locals : { code : 1 } });
      ...
      {
        // test.identical( op.exitCode, 1 )
        console.log( 'PP', op.process );
        console.log( 'PP', op.process.signal );
  ...
  function testApp()
  {
    ...
    if( !reason )
    {
      console.log( _.process.exitReason() )  /* null, normal, error, signal, code, time */
      return
    }
  ...
  function testAppSignal()
  {
    let _ = require( toolsPath );
    _.include( 'wProcess' );
    _.include( 'wFiles' );

    return _.process.kill( process.pid );
