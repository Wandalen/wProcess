...
function startConcurrent( test )
{
  ...
  .then( ( op ) =>
  {
    ...
    test.identical( op.runs[ 0 ].exitSignal, null );
    test.identical( op.runs[ 0 ].exitReason, 'normal' );
    test.identical( op.runs[ 0 ].ended, true );
    test.identical( op.runs[ 0 ].state, 'terminated' );
  ...
  .then( ( op ) =>
  {
    ...
    test.identical( op.runs[ 1 ].exitSignal, null );
    test.identical( op.runs[ 1 ].exitReason, 'normal' );
    test.identical( op.runs[ 1 ].ended, true );
    test.identical( op.runs[ 1 ].state, 'terminated' );
...
function childrenOptionFormatList( test )
{
  ...
  .then( () =>
  {
    ...
    ready.then( ( op ) =>
    {
      ...
      return children.then( ( op ) =>
      {
        if( process.platform === 'win32' ) /* xxx qqq */
        {
